...
class RiskModule:
    ...
    def filtro_seguridad(self, simbolo_interno: str) -> bool:
        ...
        # Verificación 4: Protección de Capital — Drawdown máximo flotante
        params = self.db.get_parametros()
        max_dd_pct = float(params.get("GERENTE.max_drawdown_pct", 6.7)) / 100.0
        balance = self._obtener_balance()
        limite_usd = balance * max_dd_pct
        acc_info = mt5_lib.account_info()
        if acc_info and acc_info.profit < -limite_usd:
            print(
                f"[RISK] BLOQUEO DRAWDOWN: Perdida flotante ${acc_info.profit:.2f} supera "
                f"limite ${-limite_usd:.2f} ({max_dd_pct*100:.1f}% de ${balance:.0f}). "
                f"No se abren nuevas ordenes."
            )
            return False
